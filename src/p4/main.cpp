...
struct Options
{
    ...
    int width, height;
    int num_samples;
    // allow user to specify the side length of camera len
    // to create depth of field effect for ray tracing
    int camera_len_length;
    // allow user to specify whether lights should attenuate
    // in dielectrics according to Beer's Law
    bool Beers_Law;
};
...
int main( int argc, char* argv[] )
{
    ...
    if ( !load_scene( &app.scene, opt.input_filename, 
                      opt.camera_len_length , opt.Beers_Law ) ) {
